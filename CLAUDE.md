...
./tools/sweep.sh                                                # 파라미터 스윕 (밸런스 도구)
```

스윕은 게이트가 아니다. 세팅마다 성능 엔벌로프를 재고 레이싱 라인의 목표 속도를 그 차의 실제 능력으로 다시 계산한 뒤 플라잉 랩을 돌리므로 수 분이 걸린다. 읽으라고 있는 표이지 통과/실패를 보라고 있는 것이 아니라서 `check.sh`에 넣지 않았다.
...
밸런스를 판단할 때는 `./tools/sweep.sh`를 쓴다. 게이트는 "차가 못 달리게 됐는가"만 보고, 스윕은 "어떤 세팅이 왜 빠른가"를 표로 낸다. 스윕이 하드 실패하는 경우는 PRD §27 트레이드오프 자체가 깨졌을 때뿐이다 — 특정 세팅이 완주하지 못하는 것은 실패가 아니라 결과이고, 표에 DNF로 남는다.
