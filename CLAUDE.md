...
game/ai       AI 드라이버 (라인 추종 → DriverInput). 헤드리스 전용, 렌더링 의존 없음
...
5. AI 드라이버 랩타임 회귀 (`--sim --laptime`) — 레이싱 라인으로 두 바퀴를 돌고 플라잉 랩을 기준값과 비교
...
랩타임 회귀의 기준값(`lap_probe.gd`의 `BASELINE_LAP_S`)은 **현재 차량 세팅에 묶여 있다.** 차량이나 물리를 의도적으로 바꿨다면 기준값도 함께 갱신하고, 왜 바뀌었는지를 커밋 본문에 남긴다. 설명 없이 기준값만 옮기는 것은 게이트를 끄는 것과 같다.
